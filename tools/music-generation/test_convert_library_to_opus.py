...
class ConvertLibraryToOpusTests(unittest.TestCase):
    ...
    def test_converts_to_separate_closed_world_candidate_and_preserves_ids(self, encode, probe, _canonicalize, _version):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            report = conversion.convert(source, root / "opus", max_total_bytes=100_000, pack_version="0.2.1-opus.1", app_version_requirement=">=0.2.1-beta.1, <0.3.0")
            result = root / "opus"
            manifest = json.loads((result / "manifest.json").read_text())
            asset = manifest["items"][0]["variants"][0]["asset"]
            self.assertEqual(manifest["format_version"], 2)
            self.assertEqual(manifest["pack"]["version"], "0.2.1-opus.1")
            self.assertEqual(manifest["pack"]["app_version_requirement"], ">=0.2.1-beta.1, <0.3.0")
    ...
    def test_dry_run_validates_source_and_has_deterministic_output_mapping(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            source = self.make_source(root)
            plan = conversion.dry_run(source, root / "opus", pack_version="0.2.1-opus.1", app_version_requirement=">=0.2.1-beta.1, <0.3.0")
    ...
    def test_creates_a_missing_destination_parent(self, encode, probe, _canonicalize, _version):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            conversion.convert(
                source,
                result,
                pack_version="0.2.1-opus.1",
                app_version_requirement=">=0.2.1-beta.1, <0.3.0",
            )
    ...
    def test_only_out_of_range_safe_loop_is_clamped_and_recorded(self, encode, probe, _canonicalize, _version):
        ...
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            source = self.make_source(root)
            report = conversion.convert(source, root / "opus", pack_version="0.2.1-opus.1", app_version_requirement=">=0.2.1-beta.1, <0.3.0")
    ...
    def test_budget_failure_removes_staging_and_destination(self, encode, probe, _canonicalize, _version):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            with self.assertRaisesRegex(conversion.ConversionError, "over budget"):
                conversion.convert(source, root / "opus", max_total_bytes=1, pack_version="0.2.1-opus.1", app_version_requirement=">=0.2.1-beta.1, <0.3.0")
            self.assertFalse((root / "opus").exists())
